...
describe("anchor_escrow_q1_26", () => {
  ...
  const expectClosedOrDrained = async (pubkey: anchor.web3.PublicKey) => {
    const info = await provider.connection.getAccountInfo(pubkey);
    if (info === null) return;
    expect(info.lamports).to.equal(0);
  ...
  it("Makes and refunds the escrow", async () => {
    ...
    // Check closed
    await expectClosedOrDrained(escrowPda);
    await expectClosedOrDrained(vault);
  });
